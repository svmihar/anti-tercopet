...
def main(thresholdarea: int, delaytime: int):
    while(True):
        ...
        threshold = cv2.dilate(threshold, np.ones(
            (3, 3), np.uint8), iterations=2)
        cv2.imshow('threshold', threshold)
        _, countour, heirarchy = cv2.findContours(
        ...
        cv2.putText(frame2, f'Status: {text}', (10, 40),
                    cv2.FONT_HERSHEY_SIMPLEX, 2.1, (0, 0, 255), 5)
